...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False,
                 ...
                 attention_type='divided_space_time_full'):
        super().__init__()
        self.attention_type = attention_type
        assert (attention_type in ['divided_space_time_full', 'space_only',
                                   'joint_space_time'])
        ...
        if self.attention_type == 'divided_space_time_full':
            self.temporal_norm1 = norm_layer(dim)
            self.temporal_attn = Attention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                attn_drop=attn_drop, proj_drop=drop)
            self.temporal_norm2 = norm_layer(dim)
            self.temporal_mlp = Mlp(in_features=dim,
    ...
    def forward(self, x, B, T, W):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            return x
        elif self.attention_type == 'divided_space_time_full':
            ## Temporal
            # [B, H*W*T, C]
            xt = x[:, 1:, :]
            ...
            res_temporal = rearrange(res_temporal, '(b h w) t m -> b (h w t) m',
                                     b=B, h=H, w=W, t=T)
            res_temporal = self.drop_path(
                self.temporal_mlp(self.temporal_norm2(res_temporal)))
            xt = x[:, 1:, :] + res_temporal
            ...
            xs = xt
            # [B*T, H*W, C]
            xs = rearrange(xs, 'b (h w t) m -> (b t) (h w) m', b=B, h=H, w=W,
                           t=T)
            # [B*T, 1+H*W, C]
            ...
            ## Mlp
            # [B, 1+H*W*T, C]
            x = torch.cat((init_cls_token, x), 1) + torch.cat((cls_token, res),
                                                              1)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 drop_path_rate=0.1, hybrid_backbone=None,
                 norm_layer=nn.LayerNorm, num_frames=8,
                 attention_type='divided_space_time_full', dropout=0.,
                 zero_init=True, recurrence=1):
        super().__init__()
        self.attention_type = attention_type
        self.depth = depth
        ...
        if self.attention_type == 'divided_space_time_full' and zero_init:
            for m in self.blocks.modules():
                if isinstance(m, Block):
                    nn.init.constant_(m.temporal_mlp.fc2.weight, 0)
                    nn.init.constant_(m.temporal_mlp.fc2.bias, 0)
...
class vit_full_base_patch16_224(nn.Module):
...
@MODEL_REGISTRY.register()
class vit_full_deit_tiny_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        super().__init__()
        self.pretrained = cfg.TIMESFORMER.PRETRAINED
        patch_size = 16
        self.model = VisionTransformer(
            img_size=cfg.DATA.TRAIN_CROP_SIZE,
            num_classes=cfg.MODEL.NUM_CLASSES,
            patch_size=patch_size, embed_dim=192,
...
@MODEL_REGISTRY.register()
class vit_full_deit_small_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        super().__init__()
        self.pretrained = cfg.TIMESFORMER.PRETRAINED
        patch_size = 16
        self.model = VisionTransformer(
            img_size=cfg.DATA.TRAIN_CROP_SIZE,
            num_classes=cfg.MODEL.NUM_CLASSES,
            patch_size=patch_size, embed_dim=384,
...
@MODEL_REGISTRY.register()
class vit_full_deit_base_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        super().__init__()
        self.pretrained = cfg.TIMESFORMER.PRETRAINED
        patch_size = 16
...
@MODEL_REGISTRY.register()
class vit_full_deit_rec_small_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        super().__init__()
        self.pretrained = cfg.TIMESFORMER.PRETRAINED
        patch_size = 16
        self.model = VisionTransformer(
            ...
            num_classes=cfg.MODEL.NUM_CLASSES,
            patch_size=patch_size, embed_dim=384,
            depth=cfg.TIMESFORMER.DEPTH, num_heads=6, mlp_ratio=4,
            qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            drop_rate=0., attn_drop_rate=0.,
            drop_path_rate=0.1,
            num_frames=cfg.DATA.NUM_FRAMES,
            attention_type=cfg.TIMESFORMER.ATTENTION_TYPE,
            recurrence=cfg.TIMESFORMER.RECURRENCE,
            **kwargs)
